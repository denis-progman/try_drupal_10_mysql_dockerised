...
class ExtensionSerializationTest extends UnitTestCase {
   ...
   * container's web.root and not the DRUPAL_ROOT constant if the service
   * container web.root is available.
   *
   * @covers ::__sleep
   * @covers ::__wakeup
   */
  public function testServiceAppRouteUsage() {
    ...
    // Set a dummy container web.root to test against.
    $container->setParameter('web.root', 'vfs://dummy_app_root');
    \Drupal::setContainer($container);
    // Instantiate an Extension object for testing unserialization.
    $extension = new Extension($container->getParameter('web.root'), 'module', 'core/modules/system/system.info.yml', 'system.module');
    ...
    // Change the web root and test serializing and unserializing again.
    $container->setParameter('web.root', 'vfs://dummy_app_root2');
    \Drupal::setContainer($container);
    $extension = unserialize(serialize($extension));
    $reflected_root = new \ReflectionProperty($extension, 'root');
