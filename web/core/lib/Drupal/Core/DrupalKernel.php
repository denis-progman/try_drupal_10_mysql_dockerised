...
class DrupalKernel implements DrupalKernelInterface, TerminableInterface {
   ...
   *
   * This is a nested array whose top-level keys are 'web' and 'site', denoting
   ...
   *
   * Site path is relative to the web root directory.
  ...
  public function discoverServiceProviders() {
    ...
    $this->serviceYamls['web']['core'] = 'core/core.services.yml';
    $this->serviceProviderClasses['web']['core'] = 'Drupal\Core\CoreServiceProvider';
    ...
    foreach ($module_filenames as $module => $filename) {
      ...
      if (class_exists($class)) {
        $this->serviceProviderClasses['web'][$module] = $class;
      }
      $filename = dirname($filename) . "/$module.services.yml";
      if (file_exists($filename)) {
        $this->serviceYamls['web'][$module] = $filename;
      }
    }
  ...
  protected function initializeContainer() {
    ...
    if (isset($container_definition)) {
      ...
      $container_definition['parameters']['web.root'] = $this->getAppRoot();
  ...
  protected function compileContainer() {
    ...
    foreach ($this->serviceYamls['web'] as $filename) {
      $yaml_loader->load($filename);
    }
    foreach ($this->serviceProviders['web'] as $provider) {
    ...
    $container->setParameter('web.root', $this->getAppRoot());
