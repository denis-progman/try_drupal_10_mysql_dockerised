...
class Extension {
  ...
  public function __construct($root, $type, $pathname, $filename = NULL) {
    ...
    assert($pathname === 'core/core.info.yml' || ($pathname[0] !== '/' && file_exists($root . '/' . $pathname)), sprintf('The file specified by the given web root, relative path and file name (%s) do not exist.', $root . '/' . $pathname));
    $this->root = $root;
    $this->type = $type;
  ...
  public function __sleep() {
    ...
    // Don't serialize the web root, since this could change if the install is
  ...
  public function __wakeup() {
    // Get the web root from the container. While compiling the container we
    ...
    $this->root = $container && $container->hasParameter('web.root') ? $container->getParameter('web.root') : DRUPAL_ROOT;
  }
